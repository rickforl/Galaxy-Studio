...
- [x] **Perf — nebula render** (2026-07-18, builds .45–.46): the ~140ms domain-warp field re-render (fired by "Step over time" even when idle, and mid-drag) threw `[Violation] rAF handler took Nms`. Fixed with a **drag limiter** (defer the crossfade during pan/zoom until interaction pauses) + a **chunked render** (`renderNebRows`, `NEB_CHUNK` rows/frame across ~11 frames): worst frame **143ms → ~20ms**, under the 50ms line; build stays hidden until complete.
