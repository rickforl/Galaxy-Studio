...
| **Polar/square hybrid A — square rings + spokes** | A polar frame wearing square clothing: nested **square rings** (instead of circles) + radial spokes. Addressing stays `ring × sector`, just square-shaped. | *this session's design sketch* (`polar_square_hybrid_grid_candidates`); radar-HUD ring/spoke look in `AI DISCUSSIONS/Ships_Display/Miner 2049er/Mini_Type_06_Bio_Arc_001.txt` | A 5th `type`; readout ≈ the polar fix with a square-ring distance test. The cleanest single true-hybrid look. |
| **Polar/square hybrid B — morph rings (circle→square)** | Concentric **superellipse** rings that round the core and square off toward the edge, + spokes. Prettiest, but the morph zone makes "which cell" fuzzy — decorative more than addressable. | *this session's design sketch* | A 5th `type`; superellipse `\|x/a\|^n + \|y/b\|^n = 1`, n 2→∞ per ring. Addressing TBD (that's the catch). |
| **Polar/square hybrid C — square core, polar rim** | Cartesian sectors near the core, range-rings + bearings out at the frontier — two zones stitched at a radius. | *this session's design sketch* | A 5th `type`; readout dispatches on radius (inner zone → sector name, outer → range/bearing). The most ambitious. |

**Recommended order:** Isometric first (most reused, biggest visual payoff), then
filled-cell (cheap, high utility for a *galaxy* map), then the rest. The three
**polar/square hybrids A–C** came out of a design sketch this session; the fourth
candidate — **D, a range + bearing overlay** — already shipped (see §2, *Range + bearing
overlay*), since it layers over the existing square renderer rather than being a new frame.
...
| ✅ **BUILT** — **Live cursor readout** | Grew into the full **coordinates readout**: `s2w` inverse (incl. iso un-projection), mouse / waypoint / system targets, **grid vs generation** datum, relative range+bearing, Y-up flip. In the exhibit (.10) and the galaxy_studio Grid tab. | `03_2026/canvas_Battle_Map_02.txt` (`logicalToScreen`/`screenToLogical`); `AI DISCUSSIONS/Ships_Display/Miner 2049er/Mini_Type_04_Galaxy_004.txt` (`Sector: x, y` readout) |
...
| ✅ **BUILT** — **Range + bearing overlay** | Concentric **range rings** (distance-labelled) + **compass bearing spokes** (N-up, 3-digit) over any family, own color/opacity; also appends range+bearing to the non-polar readout (`E3 · r820 · 135`). Candidate **D** of the polar/square-hybrid sketch. Built in galaxy_studio (build .05). | `EBB_TBC_V4/BR_BRAINSTORMING/3v3_SIM_canvas_20260429_04_range_rings.txt`; `Mini_Type_06_Bio_Arc_001.txt` (concentric range rings) |
